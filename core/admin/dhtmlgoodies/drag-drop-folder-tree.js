...
Modified for Oog Photo-Gallery
************************************************************************************************************/	
		
	var JSTreeObj;
	var treeUlCounter = 0;
	var nodeId = 1;
	var lastDragNode_destination = false;
	...
	function JSDragDropTree()
	{
		...
		this.floatingContainer.style.listStyle = 'none';		
	...
	*/		return false;						
		...
		moveDragableNodes : function(e)
		{
			...
			JSTreeObj.floatingContainer.style.left = dragDrop_x + 22 + 'px';
			JSTreeObj.floatingContainer.style.top = dragDrop_y - 22 + 'px';
			JSTreeObj.floatingContainer.style.zIndex = '9999';
			
			var thisObj = this;
			if(thisObj.tagName=='A')thisObj = thisObj.parentNode;
			if( thisObj.tagName=='IMG')thisObj = thisObj.parentNode.parentNode;
      var offset = $(thisObj).offset();
			...
			if(thisObj && thisObj.id)
			{
				...
				if(thisObj.tagName=='A' || JSTreeObj.dragNode_noSiblings){
					tmpImg.src = tmpImg.src.replace('ind1','ind2');	
					JSTreeObj.insertAsSub = true;
					tmpObj.style.left = (offset.left) + 'px';
					//JSTreeObj.dragNode_destination.style.borderBottom = '30px solid';
				}else{
					tmpImg.src = tmpImg.src.replace('ind2','ind1');
					JSTreeObj.insertAsSub = false;
					tmpObj.style.left = (offset.left) + 'px';
					
					if(lastDragNode_destination) lastDragNode_destination.style.borderBottom = '0'
					JSTreeObj.dragNode_destination.style.borderBottom = '30px solid yellow';
					
				}
				lastDragNode_destination = JSTreeObj.dragNode_destination;
				
				tmpObj.style.top = (offset.top + $(thisObj).height() - 5) + 'px';
				tmpObj.style.zIndex = '1';
		  ...
		  if(lastDragNode_destination) lastDragNode_destination.style.borderBottom = '0'
			...
			if(JSTreeObj.dragNode_destination){
              ...
              // No sub exists - use the appendChild method - This line should not be executed unless there's something wrong in the HTML, i.e empty <ul>
				...
				if(lis.length==0)	tmpObj.parentNode.removeChild(tmpObj);						  
        ...
        // define ajax_request for moving folder //
        var move_folder_request = false;
        move_folder_request = new XMLHttpRequest();
        move_folder_request.onreadystatechange = function() {
          if(move_folder_request.readyState == 4) {
            if(move_folder_request.status == 200) {
      
              // check for session timeout //
              if(move_folder_request.responseText.substr(0,5) == "<scri") window.location.href = 'showalbum.php?timeout=true';
      
              if(move_folder_request.responseText == 'succeed') {
      
                // moving was successful -> update foler tree //
                window.setTimeout("update_folder_tree(false, false)", 3000);

              } else {
      
                // display received error //
                $('#oogErrorBox').html(move_folder_request.responseText);
        ...
        // send move folder request //
        move_folder_request.open('GET', 'content_management_request.php?ajax_request=move_folder&folder=' + selected_folder, true);
        move_folder_request.send(null);
        ...
        aTag.onmousemove = JSTreeObj.moveDragableNodes;
				...
				if(noChildren){
					folderImg.src = this.imageFolder + this.albumImage;
				}else{
					folderImg.src = this.imageFolder + this.folderImage;
				}
				menuItems[no].childNodes[0].insertBefore(folderImg, aTag.childNodes[0]);
				if(!noDrag) {
          folderImg.style.cursor = 'move';
        } else {
          folderImg.style.cursor = 'default';
        }
        folderImg.style.marginRight = '6px';
